    ...
    <div class="container-fluid px-0 bg-gray-700">
        ...
        <div class="bg-gray-700 grid md:grid-cols-2 grid-cols-1 font-extrabold">
            <div class="mx-4 mt-5">
                ...
                <!-- Copy URl -->
                <span @click="copyURL" title="نسخ رابط السلعة" class="bg-gray-800 rounded-lg text-gray-400 p-2 mr-3 select-none cursor-pointer hover:text-yellow-500 transition duration-700">
                    <fa icon="link" class="text-xs ml-2" />
                    <span id="copied" class="text-xs">نسخ رابط السلعة</span>
                </span>

                <!-- Whatsapp -->
                <a :href="url" target="_blank" class="mr-3 text-gray-400 bg-gray-800 rounded-lg p-2 hover:text-yellow-500 transition duration-700 select-none">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-whatsapp text-green-500 inline" viewBox="0 0 16 16">
                    ...
                    <p class="mr-2 inline text-xs">استفسر عن السلعة</p>
                </a>
                ...
                <a v-if="product.review" :href="product.review" target="_blank" class="mr-3 inline-block mt-3 text-gray-400 bg-gray-800 rounded-lg p-2 hover:text-yellow-500 transition duration-700 select-none">
                    ...
                    <p class="mr-2 inline text-xs">فيديو مراجعة السلعة</p>
                </a>

                <!-- Description -->

                <div class="relative pt-3">
                    <span class="absolute bg-gray-700 px-3 text-gray-500 pos">الوصف</span>
                ...
                <div class="my-3 mr-3 opacity-90 select-none">
                    <span v-if="product.pd_stack > 0" class="bg-green-500 text-gray-900 font-light rounded-lg ml-2 px-2 py-1">متوفر</span>
                    <span v-else class="bg-red-500 text-white font-light rounded-lg ml-2 px-2 py-1">غير متوفر</span>
                    <span v-if="product.pd_state === 1" class="bg-yellow-500 text-gray-800 font-light rounded-lg ml-3 px-2 py-1">مميز</span>
                </div>
            </div>
            ...
            <div class="grid my-4 mx-5 justify-center max-h-screen">
                ...
                <div class="flex justify-center items-center pt-2">
                    ...
                    <div v-else @click='likes(product.id)' class="btn btn-sm btn-outline-secondary border flex items-center mx-1">
                        <span class="text-gray-400">اضف للمفضلة</span>
                    ...
                    <div v-if="product.pd_stack">
                        ...
                        <div v-else @click="cart(product.id)" class="btn btn-sm btn-outline-secondary border flex items-center mx-1">
                            <span class="text-gray-400">اضف للسلة</span>
                    ...
                    <p v-else class="text-yellow-500 border py-1 px-2 rounded-md mx-2 cursor-default select-none">نفذت الكمية</p>
                </div>

            </div>
        </div>

        <!-- <h1 class="text-xl text-gray-400 font-black mb-3 text-center">سلع اخرى</h1> -->
        <Title text="متعلقة" />
...
export default {
    data() {
        return {
            url: "https://api.whatsapp.com/send/?phone=964" + this.product.phone + "&text=" + this.product.message + "+http://microstoreiq.com/product/" + this.product.id,
        }
    },
    components:{
        navBar,
    ...
    methods:{
        ...
        copyURL(){
            ...
            document.getElementById('copied').innerHTML = "تم النسخ &#10004;&#65039;"
            setTimeout(() => {
